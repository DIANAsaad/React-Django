...
const normalizeModule = (module: Module) => ({
  ...module,
  module_image: module.module_image ? `${ENDPOINT}${module.module_image}` : '/achieve_a_mark.png',
});
  ...
  const addModule = useCallback(
    async (module_title: string, topic: string, module_image: File | null, lesson_pdf: File | null) => {
      try {
        ...
        setModules(prevModules => [...prevModules, normalizeModule(response.data)]);
      } catch (error: any) {
        console.error(`Error adding module:`, error.response?.data || error.message);
        alert(`An error occurred while adding the module: ${error.response?.data?.message || error.message}`);
      }
    },
    [accessToken]
  );
