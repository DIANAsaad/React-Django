...
def course_page(request, course_slug=None, course_id=None):
    if course_slug:
        course = get_object_or_404(
            Course.objects.prefetch_related("modules"), course_slug=course_slug
        )
    elif course_id:
        course = get_object_or_404(
            Course.objects.prefetch_related("modules"), id=course_id
        )
...
def course_module(request, module_slug=None, module_id=None):
    if module_slug:
        module = get_object_or_404(
            Module.objects.prefetch_related("quizzes", "flashcards"),
            module_slug=module_slug,
        )
    elif module_id:
        module = get_object_or_404(
            Module.objects.prefetch_related("quizzes", "flashcards"), id=module_id
        )
...
def study_guide(request, course_slug):
    ...
    return render(
        request,
        "main/course/study_guide.html",
        {
            "study_guide": course.study_guide,
            "id": course.id,
...
def flashcard(request, module_id):
    ...
    return render(
        request,
        "main/course/lesson_flashcard/flashcard.html",
...
def quiz_results(request, quiz_id):
    quiz = get_object_or_404(Quiz.objects.prefetch_related("quiz_attempts"), id=quiz_id)
    latest_answer_subquery = (
        Answer.objects.filter(answered_by=request.user, question=OuterRef("pk"))
    ...
    latest_attempt = (
        quiz.quiz_attempts.filter(taken_by=request.user).order_by("-taken_at").first()
    )
    quiz.latest_attempt = latest_attempt
    quiz.annotated_questions = list(questions)
    return render(request, "main/course/quiz/quiz_results.html", {"quiz": quiz})
...
def add_module(request, course_id):
    if request.method == "POST":
        ...
        form = AddModuleForm()
    return render(
        request, "main/course/add_module.html", {"form": form, "course_id": course_id}
    )
...
def delete_quiz(request):
    return delete_object(
        request, app_label="main", model_name="Quiz", object_id="quiz_id"
    )
...
def add_quiz(request, module_id):
    if request.method == "POST":
        ...
        try:
            ...
            question_time_limit = [
                None if not value else value for value in question_time_limit_set
            ]
            print(question_time_limit)
            # Validate that at least one question exists
            if not question_texts or len(question_texts) == 0:
                return HttpResponse(
                    "Error: At least one question is required.", status=400
                )
...
def quiz(request, quiz_id):
    quiz = get_object_or_404(
        Quiz.objects.prefetch_related("quiz_attempts", "questions"), id=quiz_id
    )
    user = request.user
    ...
    if quiz.attempts_allowed is None or attempts < quiz.attempts_allowed:
        if request.method == "POST":
            ...
            QuizAttempt.objects.create(
                taken_by=user, quiz=quiz, score=final_score, attempts_taken=attempts + 1
            )
        ...
        return render(
            request,
            "main/course/quiz/quiz_page.html",
            {
                "quiz": quiz,
                "formset": formset,
            },
